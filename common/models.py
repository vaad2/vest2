...
from .utils import UploadPath, lang_get, camel_to_underline
...
class FileJs(AbstractFile):
    file = models.FileField(_('file'), upload_to=UploadPath(base_path='uploads/%s/js/%s', field_name='user'))


class FileCss(AbstractFile):
    file = models.FileField(_('file'), upload_to=UploadPath(base_path='uploads/%s/css/%s', field_name='user'))


class FileImg(AbstractFile):
    file = models.FileField(_('file'), upload_to=UploadPath(base_path='uploads/%s/img/%s', field_name='user'))


class File(AbstractFile):
    file = models.FileField(_('file'), upload_to=UploadPath(base_path='uploads/%s/file/%s', field_name='user'))
...
class AbstractList(models.Model):
    ...
    pos = models.PositiveIntegerField(verbose_name=_('order'), default=0)
    ...
    def save(self, **kwargs):
        ...
        if not self.pk:
            max_pos = self.__class__.objects.aggregate(Max('order'))['pos__max']
...
class AbstractTree(BaseAbstractTree):
    ...
    class Meta:
        verbose_name = _('tree')
        verbose_name_plural = _('tree')
        ordering = ['site', 'inner_pos', 'order', 'title']
        abstract = True
...
class AbstractSimplePage(AbstractUserSiteDefaultModel):
    ...
    class Meta:
        ...
        ordering = ('position_nav', 'order', 'position_content', 'seo_title')
        abstract = True
...
#
#             ext = getattr(settings, 'DEFAULT_TEMPLATE_EXT', '.html')
#             tn = '%s/%s%s' % (namespace, base, ext)
#
...
#
#             return self.json_handler_get()({'success': success,
#                                             'data': data,
